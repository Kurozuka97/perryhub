...
// Convert bookmark to Source shape for SourceCard
...
export default function VaultModal({ open, onClose, repos, loading, settings, onSave, onSelect, onBookmark, isBookmarked, uid, status }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    if (activeTab === 'bookmarks') {
      data = (settings.bookmarks || []).map(bookmarkToSource)
    } else {
      data = repos[activeTab as Tab] || []
    }
              ...
              {TABS.map((tab) => (
                <button
                  ...
                  style={{
                    ...
                    color: activeTab === tab.id ? '#00c9c9' : '#4a8888',
                  }}
                >
                  {tab.label}
              ...
              <div className="flex flex-col items-center justify-center h-48 gap-3">
                ...
                {activeTab === 'bookmarks' && (
                  <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 9, color: '#1a2a2a', textTransform: 'uppercase', letterSpacing: 1 }}>
                    Hover a source card and click the bookmark icon
                  </p>
                )}
              </div>
            ) : (
